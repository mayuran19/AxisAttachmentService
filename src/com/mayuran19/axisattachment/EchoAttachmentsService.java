...
/**
 * An example of This class has a main method that beside the standard arguments
 * allows you to specify an attachment that will be sent to a service which will
 * then send it back.
 * 
 */
public class EchoAttachmentsService {
	...
	public DataHandler echo(DataHandler dh) {
		...
		// Attachments are sent by default back as a MIME stream if no
		// attachments were
		// received. If attachments are received the same format that was
		// received will
		...
		// Message
		// rspmsg=AxisEngine.getCurrentMessageContext().getResponseMessage();
		...
		if (dh == null)
			...
			System.err.println("Received \"" + dh.getClass().getName() + "\".");
		return dh;
	}

	/**
	 * This method implements a web service that sends back an array of
	 * attachment it receives.
	 */
	public DataHandler[] echoDir(DataHandler[] attachments) {
		...
		if (attachments == null)
			...
			System.err.println("Got " + attachments.length + " attachments!");
		return attachments;
	}
	...
	public Document attachments(Document xml) throws org.apache.axis.AxisFault,
		...
		org.apache.axis.attachments.Attachments attachments = reqMsg
				.getAttachmentsImpl();
		...
		for (int count = 1; caEl != null; caEl = getNextSiblingElement(caEl), ++count) {
			...
			if (null == p)
				throw new org.apache.axis.AxisFault("Attachment for ref='"
						+ href + "' not found.");
			String ordinalStr = getOrdinalHeaders(p);
			if (null == ordinalStr || ordinalStr.trim().length() == 0)
				throw new org.apache.axis.AxisFault(
						"Ordinal for attachment  ref='" + href + "' not found.");
			...
			if (!"text/plain".equals(p.getContentType()))
				throw new org.apache.axis.AxisFault("Attachment  ref='" + href
						+ "' bad content-type:'" + p.getContentType() + "'.");
		...
		if (!(com.mayuran19.axisattachment.TestRef.TheKey.equals(fullmsg
				.toString())))
		...
		org.apache.axis.attachments.AttachmentPart replyPart = new org.apache.axis.attachments.AttachmentPart(
				new DataHandler(
						new MemoryOnlyDataSource(
								replyJavaSerialized,
								java.awt.datatransfer.DataFlavor.javaSerializedObjectMimeType
										+ "; class=\""
		...
		msgBody.append(rootEl.getNamespaceURI()).append("\">\n")
				.append("\t<attachment href=\"")
				.append(replyPart.getContentIdRef())
				.append("\" ordinalPattern=\"").append(ordinalPattern)
				.append("\"/>\n").append("</attachments>\n");

		// Convert the string buffer to an XML document and return it.
		return org.apache.axis.utils.XMLUtils
				.newDocument(new org.xml.sax.InputSource(
						new java.io.ByteArrayInputStream(msgBody.toString()
								.getBytes())));
	}
	...
	String getOrdinalHeaders(org.apache.axis.Part p) {
		...
		for (java.util.Iterator i = p
				.getMatchingMimeHeaders(new String[] { com.mayuran19.axisattachment.TestRef.positionHTTPHeader }); i
				.hasNext();) {
