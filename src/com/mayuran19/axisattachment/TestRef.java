...
package com.mayuran19.axisattachment;
...
/**
 * An example of sending an attachment via messages. The main purpose is to
 * validate the different types of attachment references by content Id, content
 * location both absolute and relative.
 * 
 * Creates 5 separate attachments referenced differently by a SOAP document.
 * Each attachment contains a string that is assembled and tested to see if if
 * the attachments are correctly sent and referenced. Each attachment also
 * contains a mime header indicating its position and validated on the server to
 * see if mime headers are correctly sent with attachments.
 * 
...
public class TestRef {
	...
	public boolean testit() throws Exception {
		...
		myhttp.put(HTTPConstants.HEADER_CONTENT_LOCATION, baseLoc); // Send
																	// extra
																	// soap
																	// headers
		...
		/*
		 * Un comment the below to force a 100-Continue... This will cause
		 * httpsender to wait for this response on a post. If HTTP 1.1 and this
		 * is not set, *SOME* servers *MAY* reply with this anyway. Currently
		 * httpsender won't handle this situation, this will require the resp.
		 * which it will handle.
		 */
		// myhttp.put(HTTPConstants.HEADER_EXPECT,
		// HTTPConstants.HEADER_EXPECT_100_Continue);
		call.setProperty(HTTPConstants.REQUEST_HEADERS, myhttp);

		call.setTargetEndpointAddress(new URL(opts.getURL())); // Set the target
																// service host
																// and service
																// location,
		...
		AttachmentPart ap = new AttachmentPart(
				new javax.activation.DataHandler("Now is the time",
						"text/plain"));
		refs.add(ap.getContentIdRef()); // reference the attachment by
										// contentId.
		ap.setMimeHeader(positionHTTPHeader, "" + refs.size()); // create a MIME
																// header
																// indicating
																// postion.
		...
		refs.add("cid:" + setContentId); // reference the attachment by
											// contentId.
		ap.setMimeHeader(positionHTTPHeader, "" + refs.size()); // create a MIME
																// header
																// indicating
		...
		refs.add(baseLoc + "/firstLoc"); // reference the attachment by
											// contentId.
		ap.setMimeHeader(positionHTTPHeader, "" + refs.size()); // create a MIME
																// header
																// indicating
		...
		// Create an attachment referenced by relative location to a absolute
		// location.
		...
		// Create an attachment referenced by relative location to a relative
		// location.
		ap = new AttachmentPart(new DataHandler(" the aid of their country.",
				"text/plain"));
		...
		String namespace = "urn:attachmentsTestRef"; // needs to match name of
														// service.
		...
		// Now do the call....
		Object ret = call.invoke(new Object[] { new SOAPBodyElement(
				new ByteArrayInputStream(msg.toString().getBytes("UTF-8"))) });

		validate(ret, call, "12345");

		// Note: that even though the attachments are sent in reverse they are
		// still
		// retreived by reference so the ordinal will still match.
		int revc = 1;
		for (ap = (AttachmentPart) rev.pop(); ap != null; ap = rev.empty() ? null
				: (AttachmentPart) rev.pop()) {
			call.addAttachmentPart(ap);
		}

		// Now do the call....
		ret = call.invoke(new Object[] { new SOAPBodyElement(
				new ByteArrayInputStream(msg.toString().getBytes("UTF-8"))) });
	...
	void validate(Object ret, Call call, final String expOrdPattern)
			throws Exception {
		if (null == ret) {
		...
		org.apache.axis.message.RPCElement retrpc = (org.apache.axis.message.RPCElement) ((Vector) ret)
				.elementAt(0);

		Document retDoc = org.apache.axis.utils.XMLUtils
				.newDocument(new org.xml.sax.InputSource(
						new java.io.ByteArrayInputStream(retrpc.toString()
								.getBytes())));

		// get at the attachments.
		org.apache.axis.attachments.Attachments attachments = call
		...
		if (null == p)
			throw new org.apache.axis.AxisFault("Attachment for ref='" + href
					+ "' not found.");
		...
		if (!expOrdPattern.equals(ordPattern))
			throw new org.apache.axis.AxisFault(
					"Attachments sent out of order expected:'" + expOrdPattern
							+ "', got:'" + ordPattern + "'.");

		// now get at the data...
		DataHandler dh = ((org.apache.axis.attachments.AttachmentPart) p)
				.getDataHandler();
		System.err.println("content-type:" + dh.getContentType());

		java.util.Vector rspVector = null;
		Object rspObject = dh.getContent();// This SHOULD just return the vector
											// but reality strikes...
		if (rspObject == null)
			throw new AxisFault("", "Received unexpected object:null", null,
					null);
		else if (rspObject instanceof java.util.Vector)
			rspVector = (java.util.Vector) rspObject;
		else if (rspObject instanceof java.io.InputStream)
			rspVector = (java.util.Vector) new java.io.ObjectInputStream(
					(java.io.InputStream) rspObject).readObject();
		else
		...
		if (!(TheKey.equals(fullmsg.toString())))
			throw new org.apache.axis.AxisFault("Fullmsg not correct'"
					+ fullmsg + "'.");
	}
	...
	/**
	 * Give a single file to send or name a directory to send an array of
	 * attachments of the files in that directory.
	 */
		...
		for (int i = files.length - 1; i >= 0; --i) {
			...
			if (cf.isFile() && cf.canRead()) {
				...
				try {
					...
					System.err.println("Couldn't get file \"" + fname
							+ "\" skipping...");
					continue;
				}
	...
	static class MemoryOnlyDataSource extends
		...
		MemoryOnlyDataSource(byte[] in, String contentType)
				throws java.io.IOException {
			super(new java.io.ByteArrayInputStream(in), Integer.MAX_VALUE - 2,
					contentType, true);
		}

		MemoryOnlyDataSource(String in, String contentType)
				throws java.io.IOException {
